...
namespace Maynek.Notesvel.Library
{
    public class XmlProjectLoader : XmlLoaderBase<Project>
    {
        ...
        protected override void ParseDocument(XmlDocument document)
        {
            ...
            foreach (var childNode in element.ChildNodes)
            {
                if (childNode is XmlElement)
                {
                    ...
                    {
                        case "catalog":
                            this.Project.CatalogFilePath = childElement.InnerText;
                            break;
        ...
        protected override void CorrectTarget()
        {
            ...
            if (!Path.IsPathRooted(this.Project.CatalogFilePath))
            {
                this.Project.CatalogFilePath = Path.Combine(
                    this.Project.RootDirectory, this.Project.CatalogFilePath);
            }

            this.Project.SourceDirectory = Path.GetDirectoryName(this.Project.CatalogFilePath);
        }
